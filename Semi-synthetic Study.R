...
get_summary_stats_bs <- function(model, test_data, variables, current_data){
  ...
  real_sig_vbls <- variables[which(current_data$betaVec != 0)]
  if (length(real_sig_vbls) > 0){ #need to have some sig variables
  ...
  if (fls_pos == 0){ #stops doing 0/0
    prop_FP <- 0
  }
  else {
  prop_FP <- fls_pos/(length(test_data) - 1 - length(real_sig_vbls))
  }
  #True negative rate
  prop_TN <- 1 - prop_FP
  #False negative rate
  prop_FN <- 1 - prop_TP
  output <- c(AUC, prop_TP, prop_FP, prop_TN, prop_FN)
  }

  else{
    output <- c(AUC, 0, 1, 0, 1) #I think?
  }
...
get_summary_stats_glm <- function(model, testX, testY, current_data){
  ...
  return(c(AUC, prop_TP, prop_FP, prop_TN, prop_FN))
...
get_new_model75 <- function(model, trainX, trainY, testX, testY, current_data){
  ...
  if (length(variables) > 0){
    ...
    current_data$betaVec <- current_data$betaVec[variables]
...
outputs <- 5 #number of outputs to save for each model
...
analysis <- function(n){
  ...
  setwd("/project/ah/Bayesian-regression2/Test")
  save(round(results, 3), file = paste0(p_, "-", s0_, "-", correlation_, "-", muddle_, "-", sim_id, ".RData"))
}
