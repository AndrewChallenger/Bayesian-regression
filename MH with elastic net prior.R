#Creating a MH algorithm for logistic regression data with the elastic net prior
library(extraDistr)
...
post <- function(Y, X, beta, beta0, sigma, lambda1, lambda2){ 
  if (sigma > 0 & lambda1 > 0 & lambda2 > 0){
...
MH <- function(Y, X, n_iters, burnin, thin, sd){
  ...
  initial_sigma <- initial_lambda1 <- initial_lambda2 <- initial_beta0 <- 1
  
  #Counters for acceptance rate
  count <- rep(0, p+4) #for beta0, lambda1, lambda2, sigma too
  
  total_its <- burnin + n_iters*thin #total iterations
  
  #Now making matrix/vectors to save iterations
  sigma <- lambda1 <- lambda2 <- beta0 <- rep(0, total_its)
  beta <- matrix(rep(0, total_its * p), nrow = p)
  
  beta0[1] <- initial_beta0
  sigma[1] <- initial_sigma
  lambda1[1] <- initial_lambda1
  lambda2[1] <- initial_lambda2
  beta[,1] <- initial_beta
  ...
  for (n in 2:total_its){
    ...
    cur_beta0 <- beta0[n-1]
    cur_sigma <- sigma[n-1]
    cur_lambda1 <- lambda1[n-1]
    cur_lambda2 <- lambda2[n-1]
    cur_beta <- beta[,n-1]
    
    #generating new value of lambda
    new_lambda1 <- cur_lambda1 + rnorm(n=1, mean = 0, sd = sd[p+3])
    #Calculate log(A), where A is acceptance prob
    new_post <- post(Y, X, beta = cur_beta, beta0 = cur_beta0, sigma = cur_sigma, lambda1 = new_lambda1, lambda2 = cur_lambda2)
    old_post <- post(Y, X, beta = cur_beta, beta0 = cur_beta0, sigma = cur_sigma, lambda1 = cur_lambda1, lambda2 = cur_lambda2)
    logA <- new_post - old_post
    A <- exp(logA)
    
    if (runif(1) < A){
      lambda1[n] <- new_lambda1
      count[p+3] <- count[p+3] + 1 #as we've accepted
    }
    else {
      lambda1[n] <- cur_lambda1
    }
    
    new_lambda2 <- cur_lambda2 + rnorm(n=1, mean = 0, sd = sd[p+4])
    #Calculate log(A), where A is acceptance prob
    new_post <- post(Y, X, beta = cur_beta, beta0 = cur_beta0, sigma = cur_sigma, lambda1 = lambda1[n], lambda2 = new_lambda2)
    old_post <- post(Y, X, beta = cur_beta, beta0 = cur_beta0, sigma = cur_sigma, lambda1 = lambda1[n], lambda2 = cur_lambda2)
    logA <- new_post - old_post
    A <- exp(logA)
    
    if (runif(1) < A){
      lambda2[n] <- new_lambda2
      count[p+4] <- count[p+4] + 1 #as we've accepted
    }
    else {
      lambda2[n] <- cur_lambda2
    }
    
    new_sigma <- cur_sigma + rnorm(n=1, mean = 0, sd = sd[p+2])
    new_post <- post(Y, X, beta = cur_beta, beta0 = cur_beta0, sigma = new_sigma, lambda1 = lambda1[n], lambda2 = lambda2[n])
    old_post <- post(Y, X, beta = cur_beta, beta0 = cur_beta0, sigma = cur_sigma, lambda1 = lambda1[n], lambda2 = lambda2[n])
    logA <- new_post - old_post
    A <- exp(logA)
    ...
    new_post <- post(Y, X, beta = cur_beta, beta0 = new_beta0, sigma = sigma[n], lambda1 = lambda1[n], lambda2 = lambda2[n])
    old_post <- post(Y, X, beta = cur_beta, beta0 = cur_beta0, sigma = sigma[n], lambda1 = lambda1[n], lambda2 = lambda2[n])
    logA <- new_post - old_post
    A <- exp(logA)
    ...
    for (m in 1:p){ #for each variable in beta
      ...
      new_post <- post(Y, X, beta = new_beta, beta0 = beta0[n], sigma = sigma[n], lambda1 = lambda1[n], lambda2 = lambda2[n])
      old_post <- post(Y, X, beta = cur_beta, beta0 = beta0[n], sigma = sigma[n], lambda1 = lambda1[n], lambda2 = lambda2[n])
      logA <- new_post - old_post
      A <- exp(logA)
  ...
  lambda1 <- lambda1[seq(burnin+1, total_its, thin)]
  lambda2 <- lambda2[seq(burnin+1, total_its, thin)]
  ...
  return(list(beta = beta, beta0 = beta0, sigma = sigma, lambda1 = lambda1, lambda2 = lambda2, mu_beta = mu_beta, CI = CI, acc_rate = acc_rate))
}
